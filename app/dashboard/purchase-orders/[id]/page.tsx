...
export default function PurchaseOrderDetailPage({
  ...
  const handleCreateBill = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/purchase-orders/${id}/create-bill`, {
        method: "POST",
      });
      if (res.ok) {
        const data = await res.json();
        ...
        setTimeout(
          () => router.push(`/dashboard/vendor-bills/${data.bill.id}`),
          1000,
        );
      } else {
        const data = await res.json();
  ...
  if (!po) {
    return (
      <div className="empty-state">
        ...
        <Link href="/dashboard/purchase-orders" className="btn btn-secondary">
          ← Back
        </Link>
      </div>
    );
    ...
    <div>
      <div className="page-header">
        ...
        <div className="page-header-actions">
          ...
          {po.bill && (
            <Link
              href={`/dashboard/vendor-bills/${po.bill.id}`}
