...
export default function NewSalesOrderPage() {
  ...
  const [lines, setLines] = useState<LineItem[]>([{ productId: '', qty: 1, unitPrice: 0, taxPct: 0 }]);
  ...
  const addLine = () => setLines([...lines, { productId: '', qty: 1, unitPrice: 0, taxPct: 0 }]);
  const removeLine = (i: number) => {
    if (lines.length > 1) setLines(lines.filter((_, idx) => idx !== i));
  };

  const getLineSubtotal = (l: LineItem) => l.qty * l.unitPrice * (1 + l.taxPct / 100);
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-row">
            <div className="form-group">
              ...
              {errors.customerId && <span className="form-error">{errors.customerId[0]}</span>}
            </div>
            <div className="form-group">
              <label>SO Date *</label>
              ...
              <tbody>
                {lines.map((line, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <input type="number" className="form-input" value={line.taxPct} onChange={(e) => updateLine(i, 'taxPct', e.target.value)} min="0" step="0.01" />
                    </td>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '2rem' }}>
            ...
            <Link href="/dashboard/sales-orders" className="btn btn-secondary">Cancel</Link>
          </div>
        </form>
      </div>
    </div>
