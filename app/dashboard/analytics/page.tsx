        ...
        <div className="dash-card" style={{ marginBottom: '1.5rem' }}>
          ...
          <form onSubmit={handleCreate} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div className="form-group" style={{ flex: 1, minWidth: '200px', marginBottom: 0 }}>
              ...
              <input className="form-input" value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Marketing Department" />
            </div>
            ...
            <tbody>
              {accounts.map((acc) => (
                <tr key={acc.id} style={{ cursor: 'default' }}>
                  <td style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{acc.name}</td>
                  <td>
                    <span className={`badge ${acc.type === 'INCOME' ? 'badge-paid' : 'badge-unpaid'}`}>
                      {acc.type}
                    </span>
                  </td>
                  <td>{acc.budgets.length}</td>
                  <td style={{ textAlign: 'right', fontFamily: 'monospace' }}>
                    {fmt(acc.budgets.reduce((sum, b) => sum + b.plannedAmount, 0))}
                  </td>
                </tr>
              ))}
            </tbody>
