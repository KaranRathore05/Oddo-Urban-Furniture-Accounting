...
async function main() {
  ...
  const journalsData = [
    {
      name: "Sales Journal",
      type: "SALES",
      defaultAccountId: accountMap["Sales Income"].id,
    },
    {
      name: "Purchase Journal",
      type: "PURCHASE",
      defaultAccountId: accountMap["Purchase Expense"].id,
    },
    {
      name: "Bank Journal",
      type: "BANK",
      defaultAccountId: accountMap["Bank"].id,
    },
    {
      name: "Cash Journal",
      type: "CASH",
      defaultAccountId: accountMap["Cash"].id,
    },
  ];
  ...
  for (let i = 1; i <= 60; i++) {
    ...
    const po = await prisma.purchaseOrder.create({
      data: {
        number: `PO-2026-${String(i).padStart(3, "0")}-${Math.floor(Math.random() * 10000)}`,
        vendorId: vendor.id,
        date: date,
        status: "BILLED",
        lines: {
          create: [{ productId: product.id, qty, unitPrice: product.cost }],
        },
      },
    });
    ...
    const bill = await prisma.vendorBill.create({
      data: {
        number: `BILL-2026-${String(i).padStart(3, "0")}-${Math.floor(Math.random() * 10000)}`,
        poId: po.id,
        vendorId: vendor.id,
        billDate: date,
    ...
    if (isPaid) {
      const paymentDate = new Date(
        date.getTime() + Math.random() * 5 * 24 * 60 * 60 * 1000,
      );

      const je = await prisma.journalEntry.create({
        data: {
          journalId: journalMap["Bank Journal"].id,
          date: paymentDate,
          ...
          lines: {
            create: [
              {
                accountId: accountMap["Creditors"].id,
                debit: totalAmount,
                credit: 0,
              },
              {
                accountId: accountMap["Bank"].id,
                debit: 0,
                credit: totalAmount,
              }, // Outflow
            ],
  ...
  for (let i = 1; i <= 80; i++) {
    ...
    const so = await prisma.salesOrder.create({
      data: {
        number: `SO-2026-${String(i).padStart(3, "0")}-${Math.floor(Math.random() * 10000)}`,
    ...
    const invoice = await prisma.customerInvoice.create({
      data: {
        number: `INV-2026-${String(i).padStart(3, "0")}-${Math.floor(Math.random() * 10000)}`,
        soId: so.id,
        customerId: customer.id,
        invoiceDate: date,
        dueDate: new Date(date.getTime() + 15 * 24 * 60 * 60 * 1000),
        ...
        data: {
          ...
          lines: {
            create: [
              ...
              {
                accountId: accountMap["Debtors"].id,
                debit: 0,
                credit: totalAmount,
              },
            ],
  ...
  const analyticAccounts = await Promise.all([
    prisma.analyticAccount.create({
      data: { name: "Marketing Department", type: "EXPENSE" },
    }),
    prisma.analyticAccount.create({
      data: { name: "Sales Department", type: "INCOME" },
    }),
    prisma.analyticAccount.create({
      data: { name: "Operations", type: "EXPENSE" },
    }),
  ...
  const budgetData = [
    ...
    },
    {
      ...
      periodStart: "2026-07-01",
      periodEnd: "2026-09-30",
      responsiblePerson: "Riya",
      analyticAccountId: analyticAccounts[1].id,
    ...
    },
    {
      ...
      periodStart: "2026-07-01",
      periodEnd: "2026-09-30",
      responsiblePerson: "Nakash",
      analyticAccountId: analyticAccounts[2].id,
    ...
    },
    {
      ...
      analyticAccountId: analyticAccounts[3].id,
